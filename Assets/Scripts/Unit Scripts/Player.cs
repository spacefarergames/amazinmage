...
public class Player : Unit
{
    ...
    protected override void Start()
    {
        ...
        try
        {
            //Try to find UI Element 1. If it's not present, just catch
            UIElement1 = GameObject.Find("TutorialCanvas");
    ...
    public void FixedUpdate()
    {
        ...
        float horizontalInput = Input.GetAxis("JoyX");
        float verticalInput = Input.GetAxis("JoyY");
    ...
    private void Update()
    {
        ...
        if (Input.GetButtonDown("joystick button 0") && !PlayerInfo.SpellsFrozen && !BrewingSystem.Instance.gameObject.activeSelf) CastSpell();
        else if (Input.GetButtonDown("joystick button 2") || Input.mouseScrollDelta.y > 0)
        {
        ...
        else if (Input.GetButtonDown("joystick button 3") || Input.mouseScrollDelta.y < 0)
        {
        ...
        if (Input.GetButtonDown("joystick button 4")
            ...
            || Input.GetButtonDown("joystick button 6"))
        {
            UsePotion();
        }

        if (Input.GetButtonDown("joystick button 7") && UI_Inventory.Instance.gameObject.activeSelf)
        {
            ///Drop item.
        ...
        if (Input.GetButtonDown("joystick button 1") && NextToInteractable) InteractWithItem();

        /// The player wants to open their inventory. (Click Right stick or minus on switch)
        if (Input.GetButtonDown("joystick button 9") && UI_Inventory.Instance != null) OpenInventory();
    ...
    protected override void Move()
    {
        ...
        moveDir = new Vector3(Input.GetAxis("JoyX"),0f,Input.GetAxis("JoyY")).normalized;
    ...
    private void Rotate()
    {
        Vector3 lookDirection = new Vector3(Input.GetAxisRaw("JoyX"), 0f, Input.GetAxisRaw("JoyY"));
    ...
    private void UsePotion()
    {
        ...
        else if (Input.GetButtonDown("joystick button 5"))
        {
            ///Use potion 2
        ...
        else if (Input.GetButtonDown("joystick button 6"))
        {
            ///Use potion 3
    ...
    public override void TakeDamage(int dmgAmount)
    {
        ...
        int multiplier = FBPP.GetInt(PrefTags.DmgMulti) == 0 ? 1 : FBPP.GetInt(PrefTags.DmgMulti);
