...
public class WorldGenerator : SingletonPattern<WorldGenerator>
{
    ...
    void PrefsCheck()
    {
        WorldRows = FBPP.GetInt("PWorldRows", 4);
        WorldColumns = FBPP.GetInt("PWorldColumns", 4);

        //WorldRows = 4;
        //WorldColumns = 4;

        print("Seeds are being generated from PlayerPrefs. Sync with Steam Cloud after complete.");
         //CURRENTLY TURNED OFF
        //*
        if (FBPP.GetInt(PrefTags.UseSeed, 0) == 1)
        {
            manualSeed = true;
            seed = FBPP.GetInt(PrefTags.Seed);
        }
        else
        {
            manualSeed = false;
    ...
    {
        if (FBPP.GetInt(PrefTags.PWorldRows) == 5)
        {
