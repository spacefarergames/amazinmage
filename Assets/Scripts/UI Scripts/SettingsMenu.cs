...
public class SettingsMenu : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        resDropdown.options.Add(new TMPro.TMP_Dropdown.OptionData() { text = (FBPP.GetInt("currentWidth", defaultWidth) + "x" + FBPP.GetInt("currentHeight", defaultHeight)) });
        width.Add(FBPP.GetInt("currentWidth", defaultWidth));
        height.Add(FBPP.GetInt("currentHeight", defaultHeight));
        ...
        currentScreenMode = FBPP.GetInt("ScreenMode", 0);
        ...
        {
            Screen.SetResolution(FBPP.GetInt("currentWidth", defaultWidth), FBPP.GetInt("currentHeight", defaultHeight), FullScreenMode.FullScreenWindow);
        }
        else if (currentScreenMode == 2)
        {
            Screen.SetResolution(FBPP.GetInt("currentWidth", defaultWidth), FBPP.GetInt("currentHeight", defaultHeight), FullScreenMode.MaximizedWindow);
        }
        else if (currentScreenMode == 3)
        {
            Screen.SetResolution(FBPP.GetInt("currentWidth", defaultWidth), FBPP.GetInt("currentHeight", defaultHeight), FullScreenMode.Windowed);
        }
        #endregion
    }
    ...
    private void DropdownSelected(TMPro.TMP_Dropdown resDropdown, TMPro.TMP_Dropdown fullscreenDropdown)
    {
        ...
        if (index2 == 0)
        {
            ...
            FBPP.SetInt("currentWidth", defaultWidth);
            FBPP.SetInt("currentHeight", defaultHeight);
            FBPP.SetInt("ScreenMode", 0);
        }
        else if (index2 == 1)
        {
            Screen.SetResolution(width[index], height[index], FullScreenMode.FullScreenWindow);
            FBPP.SetInt("currentWidth", width[index]);
            FBPP.SetInt("currentHeight", height[index]);
            FBPP.SetInt("ScreenMode", 1);
        }
        else if (index2 == 2)
        {
