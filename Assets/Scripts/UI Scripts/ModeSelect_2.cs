...
public class ModeSelect_2 : MonoBehaviour
{
    ...
    {
        FBPP.SetInt(PrefTags.PWorldRows, enterRows);
        FBPP.SetInt(PrefTags.PWorldColumns, enterColumns);
        FBPP.SetInt(PrefTags.DmgMulti, playerDamageMultiplier);
        FBPP.SetInt(PrefTags.EnvironmentTheme, envTheme);
    }
    ...
    void SaveSeed()
    {
        if (enterUseSeed)
        {
            FBPP.SetInt(PrefTags.UseSeed, 1);
            FBPP.SetInt(PrefTags.Seed, enterSeed);
            print("Seed set to " + enterSeed);
        }
        else
        {
            FBPP.SetInt(PrefTags.UseSeed, 0);
